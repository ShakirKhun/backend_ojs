...
<div class="registration_slider">
	<div class="registration_slider_block">
		<h4>Создать учетную запись</h4>
		<p class="form_count">1/3</p>
		<p class="required_input">Обязательные поля</p>
		...
		<input style="display:none" type="text" placeholder="{translate key="user.username"} *" name="username" id="username" value="{$username|escape}" maxlength="32" autocomplete="username" required>
		...
		<button class="default_btn" type="submit" id="first_registrationform">Далее</button>
	...
	<div class="registration_slider_block auth_block_2">
		...
		<button class="default_btn" type="submit"">Далее</button>
		<div class="sign_in_btn">
			<p class="info_text">{translate key="plugins.themes.healthSciences.register.haveAccount"}
			{capture assign="rolesProfileUrl"}{url page="user" op="profile" path="roles"}{/capture}</p>
			<a href="{url page="login" source=$rolesProfileUrl}" class="login">
	...
	<div class="registration_slider_block auth_block_2">
		...
		<p class="required_input">Опциональные поля</p>
		<div class="input_row">
		...
		<div class="input_row">
			<input placeholder="{translate key="user.academicTitle"}" type="text" name="academicTitle" id="academicTitle" value="{$academicTitle|escape}" maxlength="40" required> 
		</div>
