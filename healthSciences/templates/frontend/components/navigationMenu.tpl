	...
	{else}
		<ul style="z-index:99;" id="{$id}">
			...
			{foreach key=field item=navigationMenuItemAssignment from=$navigationMenu->menuTree}
				...
				<li class="{*if $navigationMenuItemAssignment->navigationMenuItem->getUrl()|strpos:$urlcurrent}active{/if*}"><a href="{$navigationMenuItemAssignment->navigationMenuItem->getUrl()}"
					class="nav-link{if $hasChildren} dropdown-toggle{/if}">
					{$navigationMenuItemAssignment->navigationMenuItem->getLocalizedTitle()}
				</a></li>
